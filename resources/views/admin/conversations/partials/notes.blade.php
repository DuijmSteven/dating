<div class="notes box box-{!! $userClassName !!}">
                ...
                @foreach($notes as $note)
                    ...
                    @endif
                                    <div class="note_body {!! ($loop->index == 0) ? 'first' : '' !!} {!! ($loop->index == $notesAmount - 1) ? 'last' : '' !!}">
                                        <div>
                                            <form class="note_form-delete" action="{!! route('admin.conversations.notes.destroy', ['noteId' => $note->id]) !!}" method="POST">
                                                {!! csrf_field() !!}
    ...
    <div class="text-right">
        @include('admin.conversations.partials.notes-modal-button', ['userId' => $userId])
    </div>
</div>